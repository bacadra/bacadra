...
#$$ ________ def create_system _____________________________________________ #
    ...
    def create_system(self, name, base={}):
        setattr(self, 'base_'+name, base)
        setattr(self, 'system', name)
...
#$$ ________ def cc ________________________________________________________ #
...
#$$ ________ def add _______________________________________________________ #
    ...
    def add(name, value, units, overwrite=False):
        ...
        if (not overwrite) and (name in cunit.base):
            ...
            verrs.BCDR_cunit_ERROR_Already_Exists(cunit.system, name)
...
#$$ ________ def rem _______________________________________________________ #
    ...
    def rem(name, silent=True):
        ...
        # if not exists, then check silent flag
            ...
            verrs.BCDR_cunit_ERROR_Units_in_System(cunit.system, name)
...
#$$ ________ def primary ___________________________________________________ #
    ...
    def primary(self, system=None, check_system=True, inplace=False, key_prefix=''):
        ...
        if system is None: system = cunit.system
        ...
        for key,val in self._units.items():
            ...
            # elif key_prefix+key in self.base_si:
            #     base_value = self.base_si[key_prefix+key]
            # else:
...
#$$ ________ def convert ___________________________________________________ #
    ...
    def convert(self, units, fcover=False, post=True, inplace=False):
        ...
        if fcover:
            ...
            if val._units != {}:
                # raise error
                verrs.BCDR_cunit_ERROR_Cover(self._units, units, val._units)
...
#$$ ________ def drop_external _____________________________________________ #
    ...
    @staticmethod
    def drop_external(self, units=None, fcover=True, system=None):

        type_self = type(self)

        if type_self is cunit:
            return cunit.drop(self, units=units, fcover=fcover, system=system)

        elif type_self==list:
            return [cunit.drop_external(obj, units=units, fcover=fcover, system=system) for obj in self]

        elif type_self==tuple:
            return tuple(cunit.drop_external(obj, units=units, fcover=fcover, system=system) for obj in self)

        elif type_self==np.ndarray:
            return np.array([cunit.drop_external(obj, units=units, fcover=fcover, system=system) for obj in self])
...
#$$$ ____________ def __pow__ / __rpow__ / __ipow__ ________________________ #
    ...
    def __rpow__(self, othe):
        ...
        if self._units != {}:
            verrs.BCDR_cunit_ERROR_General('e0517', 'Power cunit type is not implemented')
