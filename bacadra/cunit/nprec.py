...
def to_precision(
    ...
    if nround is not None:
        value = round(value, nround)

    if precision is not None:

        is_neg, sig_digits, dot_power, ten_power = _sci_decompose(value, precision)

        if notation in ('auto', 'a'):
            delimiter = 'e' if delimiter is None else delimiter
        ...
        elif notation in ('eng', 'engineering', 'E'):
            delimiter = 'E' if delimiter is None else delimiter
            converter = _eng_notation

        elif notation in ('std', 'standard', 'f'):
            converter = _std_notation
    ...
    if precision is None:
        ...
        if not strip_zeros and nround>0:
            if value.rfind('.') >= 0:
                ...
                if nround-ad >= 0:
                    for i in range(nround-ad):
                        value += '0'
                else:
                    value = value[:nround-ad]
