'''
------------------------------------------------------------------------------
***** (S)OFiSTiK data(base) connection *****
==============================================================================
...
#$$ ________ def sofistik __________________________________________________ #

    def sofistik(self, env=None, parser=None, ursula=None, check=None, mode='e'):
        ...
        if env=='v2018':
            env = r'c:\Program Files\SOFiSTiK\2018\SOFiSTiK 2018'

        elif env=='v2016':
            env = r'c:\Program Files\SOFiSTiK\2016\ANALYSIS_33_X64'

        env = self.tools.sgc(name='sofistik:env', value=env, check=check)
        ...
        parser = self.tools.sgc(name='sofistik:parser', value=parser, check=check)


        if ursula=='ursula':
            ursula = r'ursula.exe'

        ursula = self.tools.sgc(name='sofistik:ursula', value=ursula, check=check)


        if env and parser and ursula:

            if   mode=='ep': return os.path.join(env, parser)

            elif mode=='eu': return os.path.join(env, ursula)

            elif mode=='e' : return env

            elif mode=='p' : return parser

            elif mode=='u' : return ursula




#$$ ________ def image_magick ______________________________________________ #

    def image_magick(self, env=None, magick=None, check=None, mode='e'):
        ...
        if env=='v7.0.7-Q16':
            env = r'c:\Program Files\ImageMagick-7.0.7-Q16'

        env = self.tools.sgc(name='image_magick:env', value=env, check=check)


        if magick=='magick':
            magick = r'magick.exe'

        magick = self.tools.sgc(name='image_magick:magick', value=magick, check=check)

        if env and magick:

            if   mode=='em': return os.path.join(env, magick)

            elif mode=='e' : return env

            elif mode=='m' : return magick
...
class sbase:
    ...
    setts.sofistik(env='v2018', parser='sps', ursula='ursula')

    setts.image_magick(env='v7.0.7-Q16', magick='magick')

    setts.project(folder_path=r'.\sfx', cdb_name='main.cdb')
