...
class table: #$#
    ...
    def __init__(self, wrap=True, max_width=80): #$$#
        ...
        self._deco = table.VLINES | table.HLINES | table.BORDER | \
            table.HEADER
        self.set_chars(['-', '|', '+', '='])
        self.reset()
    ...
    def set_deco(self, deco): #$$#
        ...
        - 'deco' can be a combinaison of:
            ...
            table.HEADER: Horizontal line below the header
            table.HLINES: Horizontal lines between rows
            table.VLINES: Vertical lines between columns
    ...
    def set_cols_width(self, array): #$$#
        ...
        n = array.count(True)
        if n > 0:
            rest_free = self._max_width - sum(array) + n - len(array)*3 - 1
            for i in range(n-1):
                index = array.index(True)
                array[index] = int(rest_free/n)
            rest_free = self._max_width - sum(array) + 1 - len(array)*3 - 1
            index = array.index(True)
            array[index] = int(rest_free)
    ...
    def _fmt_Exp(cls, x, **kw): #$$#
        ...
        - precision will be taken from `n` kw-argument.
        """
        exp = (int( math.floor( math.log10(-x if x<0 else x))))
        exp3 = (exp - ( exp % 3))*(-1 if x<0 else 1)
        exp3_sign = ('' if exp3 < 0 else ('-' if x<0 else '+'))
        x3 = x / ( 10 ** exp3)
        n = kw.get('n') - exp % 3
        x3 = '%.*f' % (n, x3)
        exp3 = '%02d' % (exp3,)
    ...
    def _splitit(self, line, isheader): #$$#
        ...
        if type(self.wrap) is bool:
            self.wrap = [self.wrap for i in self._width]
